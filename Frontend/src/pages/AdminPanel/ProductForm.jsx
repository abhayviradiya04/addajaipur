...
const ProductForm = ({ editingProduct, setEditingProduct, updateProduct }) => {
    ...
    const removeImage = (index) => {
        const newImages = product.image.filter((_, i) => i !== index); // Remove the image at the specified index
        setProduct({ ...product, image: newImages }); // Update the product state
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="product-form">
            ...
            <input type="file" name="image" placeholder="Upload Images" onChange={handleFileChange} multiple />
            ...
            <div>
                ...
                {product.image.map((img, index) => (
                    <div key={index} style={{ display: 'flex', alignItems: 'center' }}>
                        <img src={img} alt={`Product Image ${index + 1}`} style={{ width: '100px', margin: '5px' }} />
